...
| **Control** | A testable condition derived from a directive. A single directive may produce multiple controls. Controls are the units evaluated during inspection. |
...
| **Ruleset** | The set of all active directives (and their derived controls) for a domain, identified by a version number. |
...
| **Finding** | An immutable record of a deviation detected during inspection. A finding has two independent dimensions: lifecycle status (Open / Closed) and disposition (Valid / Invalid / Waived). Findings never change; they are observed and dispositioned. |
| **Remediation** | A mutable record of corrective action taken in response to a finding. Remediation has its own lifecycle: In Progress → Evidence Submitted → Pending Verification → Verified → Closed. |
...
| **Responsibilities** | Submits, modifies, retires, and reviews regulatory directives via natural language or structured requests. Defines controls derived from directives. Reviews consistency, audits the ruleset, manages versions, restores revisions, and reviews remediation evidence. |
...
| **S-16** | Compliance Representative | I want Selma to explain why a finding was raised, so that I understand the applicable control, the evidence considered, and the reasoning behind the decision. | **Given** I request an explanation for a finding. <br> **When** Selma processes it. <br> **Then** Selma identifies the applicable directive(s) and control(s), highlights the relevant portions of the submitted target, explains the reasoning that led to the finding, and suggests corrective actions where appropriate. | **P1** |

---

## Epic 4: Finding Management

*The Compliance Representative tracks findings from inspection through remediation and closure. Findings are immutable; remediation is the mutable process.*

| Story ID | Actor | User Story | Acceptance Criteria (Given-When-Then) | Priority |
| :--- | :--- | :--- | :--- | :--- |
| **S-11** | Compliance Representative | I want to view all inspection findings so that I can understand my current compliance posture. | **Given** I request my findings for a domain. <br> **When** Selma processes it. <br> **Then** Selma returns all findings with their lifecycle status (Open / Closed), disposition (Valid / Invalid / Waived), the inspection that produced them, and the associated control. | **P0** |
...
| **External Inspection Engine** | Reads any target (document, code, file, context), evaluates it against the control graph, and produces findings. | ❌ No. The user just "submits a target" and receives the inspection report. |

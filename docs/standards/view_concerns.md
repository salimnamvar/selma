...
| **Class** [`../class/`](../class/README.md) | *What types and methods* realize each package | Aggregate/entity/VO/event **members**, domain service signatures, ISP **port method shapes**, ROD use-case classes, DTOs, adapter **implements**, driving adapter types, composition-root types | Package dependency edges as the layout authority, peer inventing, zones/TLS/RPO, normative FSM tables / capability catalog / store mutability (spec/state) |
...
3. **Deployment** names zones and nodes using C4 container/store IDs; it does not redraw Component Rel graphs or class catalogs.
...
5. **Class** files use `cd_NNN_*.puml` / `CLS-NNN`. Headers: `Title`, `Source`, `C4`, `Package`, `Contract`. Domain types group by `*_domain` package names; application by `*_application`; infrastructure by `*_infrastructure`.

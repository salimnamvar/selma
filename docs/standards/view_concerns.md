...
**design_contract_version:** `1.3.0` (stamp of [`VERSION`](VERSION); whole-design freeze line)
...
1. **C4** defines IDs; package, class, deployment, state, use case, and ERD **map** to those IDs; state **annotates** machines with those IDs in headers; use case **groups** by application peers and labels non-peers `(domain)` / `(offline)`; ERD files **one-to-one** with C4 `*_store` peers.
...
7. **ERD** files use `erd_NNN_*_store.puml` / `ERD-NNN`. Headers: `Title`, `Source` (matching `spec/contracts/data_stores/{store_id}.yaml`), `C4` (store + owning `*_repository`), `Package` (owning `*_infrastructure`), `Contract`. Structure: `common/erd_styles.puml` + `erd_identities.puml` + one body per store. Entity names and column domains MUST be consistent with domain aggregates/value objects in class and with store contracts; **cross-store references are hashes/IDs only** (no drawn FK across failure domains). **No pipeline stages or FSM transition graphs** on ERD — those belong to state/activity; ERD may hold columns that *record* state (e.g. `fsm_state`, outbox `status`) as data.
8. **No PlantUML `note` blocks** on state, package topology, class, C4, deployment, use-case, or ERD diagrams — unfinished design smell. Encode as type fields, stereotypes, guards, actions, actor associations, include/extend, PK/FK/unique markers, or keep only in the Source contract. **No short-form names** in state/use-case/ERD filenames or diagram prose (`sm_`, `SoD`, `CG-IR`, `HLC`, `RO`, `CR`, …); use full words where prose appears (C4 peer IDs remain registry strings).
9. **Colors:** sole SSoT is [`common/ca_palette.puml`](common/ca_palette.puml). **MACRO** tokens are Clean Architecture layers (actors → interface → application → domain → infrastructure → frameworks → external → composition). **MICRO** tokens are cross-cutting concerns (gate, hermetic, terminal) and view-specific aliases (`$STATE_COLOR_*`, `$UC_*`, `$ERD_*`, type archetypes). Every view styles file includes the palette and re-exports aliases — never invent hex in diagram bodies.
10. If two views need the same fact, put it once under the **authority** column above and link from the others.

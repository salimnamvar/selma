...
| How does state evolve (FSMs / pipelines)? | [`state/`](state/README.md) | Extracted states/transitions from contracts | Peer inventing, packages, zones, **OpenAPI catalogs**, sequence/activity flows |
| In what order do peers collaborate? | [`sequence/`](sequence/README.md) | Message order, API path calls, ROD use-case methods | FSM charts, OpenAPI schemas, package trees, **notes** |
| What procedural control flow / swim-lanes? | [`activity/`](activity/README.md) | Decisions, fork/join, CA swim-lanes, API ingress labels | Sequence timing SSoT, FSM charts, capability tables, **notes** |
| What actor goals and ROD use cases exist? | [`usecase/`](usecase/README.md) | Actors, ROD ovals, groups, include/extend | **HTTP paths/bodies**, FSM tables, capability catalog, pipelines |
...
| What HTTP resources/methods/wire schemas? | [`api/`](api/README.md) | Paths, methods, operationIds, wire schemas, error envelope, security scheme | Peers, packages, FSM guards, capability catalog, zones, ERD keys, sequence/activity |

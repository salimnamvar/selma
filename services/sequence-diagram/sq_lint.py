...
from sq.parser import parse_sq_file
from sq.report import build_sq_report
from sq.validator import validate_sq
...
def main() -> int:
    parser = argparse.ArgumentParser(description="SQ PlantUML linter (canonical global tool)")
    ...
    for f in files:
        try:
            d = parse_sq_file(str(f))
            violations = validate_sq(d)
            rep = build_sq_report(d, violations)

...
class JsonRuleRepository(RuleRepository):
    ...
    def __init__(
        self, a_rules_dir: Path | None = None
    ) -> None:
        self._rules_dir = a_rules_dir or _DEFAULT_RULES_DIR
        self._cached_rules: (
            tuple[RuleDefinition, ...] | None
        ) = None
    ...
    def find_all(
        ...
        """
        b_continue = True
        result: Result[tuple[RuleDefinition, ...]] = (
    ...
    def find_by_id(
        ...
        if b_continue:
            ...
            for a_rule in all_rules_result.value:
                if (
                    b_continue
                    and RuleId(a_rule.lineage_id) == a_id
                ):
                    b_continue = False
                    found = a_rule
            if b_continue:
                result = Result.failure(
                    f"Rule not found: {a_id}"
                )
    ...
    def find_by_codes(
        ...
        if b_continue:
            ...
            matched = tuple(
                a_rule
                for a_rule in all_rules_result.value
                if a_rule.lineage_id in codes_set
                or a_rule.id in codes_set
            )
    ...
    def _load_all_rules(
        ...
        if b_continue:
            json_files = sorted(
                self._rules_dir.glob("*.json")
            )
            for a_file in json_files:
                if b_continue:
                    load_result = self._load_rule_file(
                        a_file
                    )
                    if b_continue and load_result.is_failure():
                        b_continue = False
        ...
        try:
            ...
            if b_continue:
                ...
                for a_raw in raw_rules:
                    if b_continue:
                        parse_result = self._parse_rule(
                            a_raw
                        )
                        if (
                            b_continue
                            and parse_result.is_failure()
                        ):
                            b_continue = False
                            result = Result.failure(
    ...
    def _parse_rule(
        ...
        if b_continue and not rule_type:
            b_continue = False
            result = Result.failure(
                f"Rule {lineage_id} missing type"
            )
        ...
        if b_continue and not message:
            b_continue = False
            result = Result.failure(
                f"Rule {lineage_id} missing message"
            )
        ...
        if b_continue and not evaluator_type:
            b_continue = False
            result = Result.failure(
                f"Rule {lineage_id} missing evaluator_type"
            )

        evaluator_config = EvaluatorConfig()
        ec_raw = a_raw.get("evaluator_config", {})
        if b_continue:
            ec_result = self._parse_evaluator_config(
                ec_raw
            )
        ...
        depends_on = tuple(
            a_raw.get("depends_on", [])
        )
        conflicts_with = tuple(
            a_raw.get("conflicts_with", [])
        )
        ...
        weight = (
            Severity(weight_str)
            if weight_str in valid_weights
    ...
    def _parse_evaluator_config(
        ...
        forbidden_functions = tuple(
            a_raw.get("forbidden_functions", [])
        )
        resource_calls = tuple(
            a_raw.get("resource_calls", [])
        )
        execute_methods = tuple(
            a_raw.get("execute_methods", [])
        )
        sql_keywords = tuple(
            a_raw.get("sql_keywords", [])
        )
        io_calls = tuple(a_raw.get("io_calls", []))
        check_first_arg = a_raw.get(
            "check_first_arg", {}
        )
        count = a_raw.get("count", {})
        message_template = a_raw.get(
            "message_template", ""
        )
        exempt_dunders = a_raw.get(
            "exempt_dunders", True
        )
        exempt_generators = a_raw.get(
            "exempt_generators", True
        )
        exempt_names = tuple(
            a_raw.get("exempt_names", [])
        )
        ...
        if sub_raw:
            ...
            for a_sub in sub_raw:
                sub_result = (
                    self._parse_evaluator_config(a_sub)
                )
                if (
                    b_continue
                    and sub_result.is_failure()
                ):
                    b_continue = False
                    msg = (
                        f"Sub-evaluator: "
                        f"{sub_result.message}"
                    )
                    result = Result.failure(msg)
                if (
                    b_continue
                    and sub_result.is_success()
                ):

...
class LineageValidator:
    ...
    def validate(self, a_graph: DirectiveGraph) -> list[LineageViolation]:
        ...
        violations.extend(self._validate_self_references(a_graph))
        violations.extend(self._validate_parent_references(a_graph))
        violations.extend(self._validate_operation_consistency(a_graph))
        violations.extend(self._validate_cycles(a_graph))
        violations.extend(self._validate_temporal_ordering(a_graph))
        violations.extend(self._validate_ancestry_depth(a_graph))
        return violations

    def _validate_self_references(self, a_graph: DirectiveGraph) -> list[LineageViolation]:
        """A rule MUST NOT list its own lineage_id or id as a parent."""
        violations: list[LineageViolation] = []
        for d in a_graph.directives:
            ...
            if d.lineage_id in d.lineage.parent_lineage_ids:
                violations.append(
                    LineageViolation(
                        directive_id=d.id,
                        message=(f"Rule references itself in parent_lineage_ids (lineage_id={d.lineage_id!r})"),
                    )
                )
    ...
    def _validate_parent_references(self, a_graph: DirectiveGraph) -> list[LineageViolation]:
        """Every lineage parent ID must reference a directive in the graph."""
        known_ids = a_graph.execution_ids()
        known_lineage_ids = a_graph.lineage_ids()
        violations: list[LineageViolation] = []
        for d in a_graph.directives:
            if d.lineage is None:
                continue
            for pid in d.lineage.parent_lineage_ids:
                if pid not in known_lineage_ids:
                    violations.append(
                        LineageViolation(
                            directive_id=d.id,
                            message=(f"Lineage parent_lineage_id {pid!r} does not exist in the graph"),
                        )
                    )
            for pid in d.lineage.parent_execution_ids:
                if pid not in known_ids:
                    violations.append(
                        LineageViolation(
                            directive_id=d.id,
                            message=(f"Lineage parent_execution_id {pid!r} does not exist in the graph"),
                        )
                    )
        return violations

    def _validate_operation_consistency(self, a_graph: DirectiveGraph) -> list[LineageViolation]:
    ...
    def _validate_cycles(self, a_graph: DirectiveGraph) -> list[LineageViolation]:
    ...
    def _validate_temporal_ordering(self, a_graph: DirectiveGraph) -> list[LineageViolation]:
        """Child lineage.timestamp MUST be ≥ max(parent lineage timestamps)."""
        by_exec = {d.id: d for d in a_graph.directives}
        violations: list[LineageViolation] = []
        for d in a_graph.directives:
            if d.lineage is None:
                continue
    ...
    def _validate_ancestry_depth(self, a_graph: DirectiveGraph) -> list[LineageViolation]:
        ...
        violations: list[LineageViolation] = []
        for d in a_graph.directives:
            depth = self._measure_depth(d.lineage_id, parent_map, a_visited=set())
            if depth > MAX_ANCESTRY_DEPTH:
                violations.append(
                    LineageViolation(
                        directive_id=d.id,
                        message=(f"Lineage ancestry depth {depth} exceeds maximum {MAX_ANCESTRY_DEPTH}"),
                    )
                )
        return violations
    ...
    def _measure_depth(
        self,
        a_node_id: str,
        a_parent_map: dict[str, list[str]],
        a_visited: set[str],
    ) -> int:
        ...
        """
        if a_node_id in a_visited:
            result = 0
        else:
            parents = a_parent_map.get(a_node_id, [])
            if not parents:
                result = 0
            else:
                a_visited = a_visited | {a_node_id}
                result = 1 + max(self._measure_depth(pid, a_parent_map, a_visited) for pid in parents)
        return result

    @staticmethod
    def _cyclic_nodes(a_adjacency: dict[str, list[str]]) -> list[str]:

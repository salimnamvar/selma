...
class Lineage(BaseModel):
    ...
    def _validate_parent_cardinality(self) -> Lineage:
        ...
        if self.operation in (LineageOperation.FORK, LineageOperation.SPLIT):
            if n_lineage != 1:
                raise ValueError(
                    f"{self.operation} requires exactly 1 parent_lineage_id; got {n_lineage}"
                )
        elif self.operation == LineageOperation.MERGE:
            if n_lineage != 2:
                raise ValueError(
                    f"merge requires exactly 2 parent_lineage_ids; got {n_lineage}"
                )

        if n_exec != n_lineage:
            raise ValueError(
                f"parent_execution_ids length ({n_exec}) must match "
                f"parent_lineage_ids length ({n_lineage})"
            )
        return self
